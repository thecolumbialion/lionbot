...
def offcampus_dining_request_msg(result, latitude=40.806209, longitude=-73.961733):
		"""Interface function"""
		try:
...
def get_yelp_info(term="", latitude=40.806209, longitude=-73.961733):
	...
	if 'businesses' not in results:
		element = Template.GenericElement(title="JJ's Place", subtitle="A Columbia late night classic", item_url="bit.ly/2shtorh",
                                            image_url="bit.ly/2shtorh",
                                            buttons=[
                                                  {'type': 'web_url', 'title': 'Learn More', 'value': "bit.ly/2rcSpAB"}])
		recommendations_list.append(element)
		return recommendations_list
	query_limit = len(results['businesses']) if len(results['businesses']) < 4  else query_limit 
	for i in range(query_limit):
		...
		element = Template.GenericElement(title=business_name, subtitle=business_subtitle, item_url=yelp_link,
                                            image_url=business_image,
                                            buttons=[
                                                  {'type': 'web_url', 'title': 'Learn More', 'value': yelp_link}])
		recommendations_list.append(element)
	return recommendations_list
