...
def get_shows():
	#make a dictionary where key is show name and value is a dictionary of show info
	shows = {}
	...
	for row in test[0].find_all("tr"):
		...
		#make dictionary of values related to a specifc show and add to dictionary for all shows
		show_dict = make_show_dictionary(theater,close_date, performance_schedule, rush_policy, rush_link)
...
def make_show_dictionary(theater,close_date, performance_schedule, rush_policy, rush_link):
	show_dict = {}
...
def search_show(show_name, desired_info=['rush_policy']):
	shows = get_shows()
	#list of shows to check based on show_name given by user (in case they don't give full show title)
	check_shows = []
	statement = ""
	if len(show_name) == 0:
	...
	if len(desired_info) == 0:
		...
		try:
			...
			if len(found_shows) == 0:
				message = "Looks like %s is not currently a show on Broadway." % (show_name)
				return message
			...
			for value in desired_info:
				try: 
					...
					result = "Looks like I couldn't find out more about %s for you." % (value)
					#bot.send_text_message(recipient_id, result)
					return result
		except: 
