...
def subwaystatus():
    ...
    for name in sorted(subwayDictionary.keys()):
        line ='%s is experiencing: %s' % (subwayDictionary[name].getName(), subwayDictionary[name].getStatus())
        mta_web_link = "(http://www.mta.info/status/subway/" + subwayDictionary[name].getName() + ")"
