...
def make_lion_feed():
    ...
    for feed in thelion['entries']:
        if lion_counter > 4:
            ...
            news_item = Template.GenericElement(title=feed['title'], subtitle=feed['description'], item_url=feed['link'],
                                              image_url=image,
                                              buttons=[
...
def make_spec_feed():
    ...
    for line, article in enumerate(test_names, 0):
        ...
        news_item = Template.GenericElement(title=title, subtitle=byline, item_url=link,
